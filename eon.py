...
class EonAgent:
 ...
 # --- ORIGINAL SELF-WRITING FUNCTION (UNSAFE - FOR DEMONSTRATION ONLY) ---
        ...
        *******************************************************************
        ...
        if os.path.exists(self.script_file):
            try:
                ...
                logger.error(f"Error reading script file {self.script_file}: {e}")
                script_content = "" # Assume empty if unreadable
        else:
            script_content = ""

        if f"def {function_name}(" not in script_content:
            logger.info(f"[Unsafe Demo] Function '{function_name}' not found in script file. Generating code via LLM...")
            prompt = (f"Write a complete Python function called '{function_name}' that {function_description}.\n"
                      "Include necessary imports if any within the function scope or assume standard libraries are imported.\n"
                      "Return ONLY the raw Python code for the function definition block, starting with 'def'. Do not include explanations or ```python markers.")

            # Use a potentially different LLM call if generation needs specific parameters
            function_code = self.call_llm(prompt, temperature=0.5, max_tokens=250) # More tokens for code, lower temp?

            if function_code and function_code.strip().startswith("def "):
                logger.info(f"[Unsafe Demo] LLM generated code snippet starting with: {function_code[:100]}...")
                try:
                    ...
                    logger.warning("Appended code is NOT dynamically loaded or executed in the current run.")
                except IOError as e:
                    logger.error(f"[Unsafe Demo] Error writing generated code to {self.script_file}: {e}")
            else:
                 logger.error(f"[Unsafe Demo] LLM did not return valid-looking function code for '{function_name}'. Received: {function_code}")
        else:
